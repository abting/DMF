...
void setup()
{
  ...
  initializeMax(max7300_02);
  clearLED();
  Serial.begin(9600);
  Serial.write("Enter number");
}
...
void loop()
{
  ...
  //  while(endT == false){
 if(Serial.available()>0 && !complete){         //  Can no longer read if end transmission signal has been read "1002" - This may be unnecessary 
  char  dataRead [100];
  Serial.println("Entered a number");           //Testing purposes only
      Serial.readBytes(dataRead,100);           //reads upto 1000 bytes
      //endT=true;  
  //}
  
  String StArray [5];    //Stores each electrode into a String
  int numArray[5]; 
  
  
  int count = 0;        //Holds the number of elements that are stored
  ...
  while(data!=NULL)// && dataFinish != true)
   ...
   // while (!complete)
    //{
      for (int i=0;i<count;i++)     //going through the array,      **You must add a "," at the end of every string NEED TO DEBUG**
      {
     ...
     Serial.write("2");
     memset(&dataRead[0], 0, sizeof(dataRead));

    //char *begin  = &dataRead[0];
    //char *end = begin + sizeof(dataRead);
    //s/td::fill(begin, end, 0);
     //Serial.write("2");
   //}
}
